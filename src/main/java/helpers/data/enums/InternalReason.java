...
public enum InternalReason {
    ACCOUNT_CREATION(
            "ACCOUNT_CREATION",
            "Due to suspicious activity, the client is not permitted to create additional trading accounts.",
            false,
            "RESTRICTION"),

    INTERNAL_TRANSFER(
            "INTERNAL_TRANSFER",
            "In order to prevent the client from transferring illicit funds from one trading account to another, we are blocking all internal transfers under his user.",
            false,
            "RESTRICTION"),

    DEPOSITS(
            "DEPOSITS",
            "Due to suspicions of illicit activity, the client should not be allowed to deposit funds. This can be due to several reasons, such as money laundering, chargebacks, credit card fraud or any of the trading fraud pattern for which funds are required.",
            false,
            "RESTRICTION"),

    WITHDRAWALS(
            "WITHDRAWALS",
            "Clients who make guaranteed, illegal profits should not be allowed to withdraw funds.",
            false,
            "RESTRICTION"),

    LOGIN_CRM(
            "LOGIN_CRM",
            "Considering the severity of certain clients' actions, their accounts need to be blocked completely. This can be relevant for more serious cases of Market manipulation, thin liquidity scalping, gap trading and more.",
            false,
            "RESTRICTION"),

    CLOSE_ONLY_MODE(
            "CLOSE_ONLY_MODE",
            "Client is abusing our trading environment, so we are not allowing them to open new positions. We still need to wait for them to close the open ones. This restriction can be relevant for any trading fraud case.",
            false,
            "RESTRICTION"),

    MANUAL_WITHDRAWAL_REVIEW(
            "MANUAL_WITHDRAWAL_REVIEW",
            "Client is suspected of taking advantage of our company to gain illegal profits, so we do not wish to allow them to withdraw funds without manual verification and possible deduction of said profits prior to withdrawal.",
            false,
            "RESTRICTION"),

    CREDIT_AND_BONUS(
            "CREDIT_AND_BONUS",
            "Client is suspected of trying to take advantage of our various credit/bonus offerings, such as deposit bonuses, no deposit bonuses, loss vouchers, etc., and thus should not be eligible for these.",
            false,
            "RESTRICTION"),
    ...
    RAF_ABUSE(
            "RAF_ABUSE",
            "Clients abuse our Refer A Friend (RAF) promotion by registering a large number of fake accounts in order to get rebates after them",
            false,
            "FRAUD"),
    ...
    POTENTIAL_ABUSE(
            "POTENTIAL_ABUSE",
            "Client is displaying suspicious patterns of behaviour, however we cannot (yet) classify him as any of the other categories, this serves as a general 'risky user flag'.",
            true,
            "FRAUD"),

    LOOPHOLE_ABUSE("LOOPHOLE_ABUSE", "Client is abusing any kind of loophole to arbitrage.", false, "FRAUD"),
    ...
    ANOMALOUS_PROFIT(
            "ANOMALOUS_PROFIT",
            "Clients have a tendency to utilize sources/methods outside of our scope to generate large amounts of risk free profits.",
            true,
            "FRAUD"),

    NEWS_TRADER_BEFORE(
            "NEWS_TRADER",
            "Client is taking advantage negative balance protection to open positions by the news release with high leverage. Then client can make high profit with limited cost.",
            true,
            "FRAUD"),
    ...
    POTENTIAL_CHARGEBACK(
            "CHARGEBACK",
            "Verification documents needed (e.g., deposit transaction proof, card photo). Please instruct the client to check their email and wait for the relevant update. Please reach out to AFOP for further clarification.",
            true,
            "FRAUD"),

    EXCHANGER(
            "EXCHANGER",
            "Accounts are restricted due to concerns of possible money laundering, upgrader-type activity, or non-trading usage. Please advise the client to check inactivity behaviour and trading activity in the account.",
            true,
            "FRAUD"),
    ...
    public static List<InternalReason> getNonExceptionalReasons(String type) {
        return Arrays.stream(values())
                .filter(r -> !r.isExceptional() && r.getType().equals(type))
