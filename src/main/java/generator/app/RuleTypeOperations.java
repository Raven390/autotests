...
final class RuleTypeOperations {
    ...
    public RuleTypeOperations(String packagePrefix) {
        annotatedClasses = new Reflections(packagePrefix).getTypesAnnotatedWith(RuleTestData.class).stream().collect(groupingBy(cl -> cl.getAnnotation(RuleTestData.class).value(), toSet()));
    }

    public Optional<MethodInfo> getMethodInfo(String ruleName, Predicate<? super Method> predicate) {
        return annotatedClasses.getOrDefault(ruleName, emptySet()).stream().map(Class::getMethods).flatMap(Arrays::stream).filter(predicate).findFirst().map(method -> {
