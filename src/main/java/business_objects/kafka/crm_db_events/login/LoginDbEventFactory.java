...
public class LoginDbEventFactory {
    ...
    private static LoginDbEventMetadata generateLoginDbEventMetadata() {
        return new LoginDbEventMetadata(
                Utils.getCurrentTimestampDbFormat(), "test_record_type", "login", "test_partition_key_type", "test_schema_name", "tb_user_login_info"
        );
    }
    ...
    private static LoginDbEventData generateLoginDbEventData() {
        return new LoginDbEventData(
                Utils.getCurrentTimestampDbFormat(), getRandomIntPositive(), "test_brand", "test_ip_address", "test_cid", "test_cookie"
        );
    }
