...
public class RuleExecutionsRequests {
    ...
    public static Response putRuleExecutionsRequest(PutRuleExecutionsBody putRuleExecutionsBody) throws IOException {
        ...
        return new HttpHelper()
                .sendPutRequest(
                        PAYMENT_GATE_SERVICE_BASE_PATH
                                + PAYMENT_GATE_RULE_EXECUTIONS_PATH.replace(
                                        "{paymentId}",
                                        putRuleExecutionsBody.getPaymentId().toString()),
                        headers,
                        null,
                        putRuleExecutionsBody);
    }
        ...
        return new HttpHelper()
                .sendPostRequest(
                        PAYMENT_GATE_SERVICE_BASE_PATH
                                + PAYMENT_GATE_RULE_EXECUTIONS_PATH.replace(
                                        "{paymentId}",
                                        postRuleExecutionsBody.getPaymentId().toString()),
                        headers,
                        null,
                        postRuleExecutionsBody);
    }
    ...
    public static Response getRuleExecutionsRequest(String id, Map<String, Object> paramsMap) throws IOException {
        ...
        return new HttpHelper()
                .sendGetRequest(
                        PAYMENT_GATE_SERVICE_BASE_PATH + PAYMENT_GATE_RULE_EXECUTIONS_PATH.replace("{paymentId}", id),
                        headers,
                        paramsMap);
    }
    ...
    public static Response getRuleExecutionsByUcidRequest(String ucid, Map<String, Object> paramsMap)
        ...
        return new HttpHelper()
                .sendGetRequest(
                        PAYMENT_GATE_SERVICE_BASE_PATH
                                + PAYMENT_GATE_RULE_EXECUTIONS_BY_UCID_PATH.replace("{ucid}", ucid),
                        headers,
                        paramsMap);
    }
}
