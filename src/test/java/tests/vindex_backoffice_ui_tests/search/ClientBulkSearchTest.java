...
class ClientBulkSearchTest extends TestBaseWeb {
    ...
    void searchBySingleAccountTest() {
        ...
        assertThat("Verify client ID is present", searchResults.getFirst(), containsString(String.valueOf(crmTbUser1.userId)));
    ...
    void searchByMultipleAccountsSameClientTest() {
        ...
        String searchQuery = String.join(" ", account1.serverName, String.valueOf(account1.account), account1Additional.serverName, String.valueOf(account1Additional.account)
        );
        ...
        assertThat("Verify first account is present", searchResults.stream().anyMatch(r -> r.contains(account1.account.toString())), is(true));

        // Verify second account
        assertThat("Verify second account is present", searchResults.stream().anyMatch(r -> r.contains(account1Additional.account.toString())), is(true));

        // Both should belong to the same user
        searchResults.forEach(result -> assertThat("Verify all results belong to same client", result, containsString(String.valueOf(crmTbUser1.userId))
        )
        );
    }
    ...
    void searchByMultipleAccountsDifferentClientsTest() {
        ...
        String searchQuery = String.join(" ", account1.serverName, String.valueOf(account1.account), account2.serverName, String.valueOf(account2.account)
        );
        ...
        assertThat("Verify client1 account is present", searchResults.stream().anyMatch(r -> r.contains(account1.account.toString()) && r.contains(String.valueOf(crmTbUser1.userId))
        ), is(true));

        // Verify client2 account is present
        assertThat("Verify client2 account is present", searchResults.stream().anyMatch(r -> r.contains(account2.account.toString()) && r.contains(String.valueOf(crmTbUser2.userId))
        ), is(true));
    }
    ...
    void searchByMixedValidInvalidAccountsTest() {
        ...
        String searchQuery = String.join(" ", account1.serverName, String.valueOf(account1.account), "InvalidServer 999999999", account2.serverName, String.valueOf(account2.account)
        );
        ...
        assertThat("Verify client1 account is present", searchResults.stream().anyMatch(r -> r.contains(account1.account.toString())), is(true));
        assertThat("Verify client2 account is present", searchResults.stream().anyMatch(r -> r.contains(account2.account.toString())), is(true));
    }
    ...
    void verifySearchResultsColumnsTest() {
        ...
        assertThat("Verify user full name is displayed", firstResult, containsString(String.join(" ", crmTbUser1.firstName, crmTbUser1.lastName))
        );
    ...
    void searchWithDifferentSeparatorsTest() {
        ...
        String[] searchQueries = {searchQueryAccount1,  // space
        ...
        for (String query : searchQueries) {
            ...
            List<String> searchResults = searchPage.getSearchResults();
            assertThat("Verify search works with different separator: " + query, searchResults, hasSize(greaterThan(0)));
            assertThat("Verify correct account found", searchResults.getFirst(), containsString(account1.account.toString()));
        }
    }
    ...
    void verifyAllTableColumnsTest() {
        ...
        assertThat("Verify account info contains account number", row.accountInfo, containsString(account1.account.toString()));
        ...
        assertThat("Verify country", row.country.toUpperCase(), is(client1.getCountry().toUpperCase()));
        ...
        assertThat("Verify balance contains 101.5", row.balance, containsString(decimalFormat.format(mtAccount1.balance)));
        ...
        if (!row.ibCpa.isEmpty()) {
            ...
            assertThat("Verify IB/CPA contains IB ID", row.ibCpa, containsString(ibRelation.getDirectIbRebateAccount().toString()));
        }
        ...
        assertThat("Verify last login contains date", row.lastLogin,
    ...
    void verifyRestrictionsInBehaviorTooltipTest() throws Exception {
        ...
        PostRestrictionRequestBody postRestrictionRequestBody = new PostRestrictionRequestBody(
                client1.getUcid(), restriction.getCode(), restriction.getType(), account1.account, account1.serverIdSt, "Search Test", new PostRestrictionRequestBody.UpdatedBy(updatedBySystem, updatedByUser)
        );
        ...
        assertThat("Verify restriction is displayed in tooltip: %s".formatted(restriction.getName()), displayedRestrictions, hasItem(restriction.getName()));
    ...
    void verifyBehaviorChangeWithAlertAndFraudTest() throws Exception {
        ...
        List<FraudType> confirmedFrauds = List.of(
                FraudType.HEDGING, FraudType.CPA_ABUSE, FraudType.BONUS_ABUSE
        );
        ...
        for (FraudType fraud : confirmedFrauds) {
            assertThat("Verify fraud type is displayed in behavior: %s".formatted(fraud.getName()), row.behavior, containsString(fraud.getName()));
        }
    }
}
