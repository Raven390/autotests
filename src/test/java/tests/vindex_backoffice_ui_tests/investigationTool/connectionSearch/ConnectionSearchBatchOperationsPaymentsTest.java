...
class ConnectionSearchBatchOperationsPaymentsTest extends TestBaseWeb {
    ...
    void verifyFraudRestrictionInConnectionSearchPaymentsTest() throws Exception {
        ...
        assertThat(
                ...
                connectionPage.getFraudTypesList(),
                everyItem(is(in(FraudType.getVisibleFraudTypeNamesList()))));
        ...
        assertThat(
                "Verify record in ar.abuser_fraud_type has correct fraud",
                fraud.getFraudTypeCode(),
                is(fraudType.getCode()));
        assertThat(
                "Verify record in ar.abuser_fraud_type has correct status",
                fraud.getStatus(),
                is(fraudTypeStatus.getStatus()));
        assertThat("Verify record in ar.abuser_fraud_type has correct comment", fraud.getComment(), is(COMMENT));
        assertThat(
                "Verify source in ar.abuser_fraud_type is default value: Vindex", fraud.getFraudSource(), is("Vindex"));
        assertThat(
                "Verify no deductions were created",
                getObjectsFromDB(
                        DbName.POSTGRES,
    ...
    void verifyFraudRestrictionInConnectionSearchPaymentsSeniorTest() throws Exception {
        ...
        List<AbuserFraudType> frauds = getObjectsFromDB(
                DbName.POSTGRES,
                AR_ABUSER_FRAUD_TYPE_TABLE_NAME,
                String.format("ucid = '%s'", connectedClient2.getUcid()),
                AbuserFraudType.class);
        assertThat("Verify there is one record in ar.abuser_fraud_type", frauds.size(), is(1));
        AbuserFraudType fraud = frauds.getFirst();
        assertThat(
