...
public class FirstLoginTest extends TestBaseWeb {
    ...
    private static final String USER_ID_WHERE_STATEMENT = String.format(
            "user_id = (select id from %s where first_name = '%s' and last_name = '%s')",
            BO_BACKOFFICE_USER_TABLE_NAME, uiUser.getFirstName(), uiUser.getLastName());
    private static final String BO_USER_WHERE_STATEMENT =
            String.format("first_name = '%s' and last_name = '%s'", uiUser.getFirstName(), uiUser.getLastName());
