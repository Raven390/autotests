...
public class AutoCleanPotentialStatusTest {
    ...
    static void setup() throws Exception {
        insertObjectsToDb(CRM_USER_TABLE_NAME, List.of(crmTbUserWithFraud, crmTbUserWithoutFraud, crmTbUserWithCleanedFraud));
    ...
    void shouldCleanUser_whenClientStatusIsPotentialAndNoFraudOrRestriction() throws Exception {
        ...
        String whereUcid = String.format("ucid IN ('%s', '%s', '%s')", crmTbUserWithFraud.ucid, crmTbUserWithoutFraud.ucid, crmTbUserWithCleanedFraud.ucid);
        ...
        Optional<Abuser> cleanedAbuserOptional = abusers.stream().filter(abuser -> abuser.getStatus().equals(CLEANED.getStatus())).findFirst();
        ...
        List<Abuser> potentialList = abusers.stream().filter(abuser -> !abuser.getStatus().equals(CLEANED.getStatus())).toList();
