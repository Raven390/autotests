...
class LarkBotAutoChangingHoldingTest extends TestBaseWeb {
    ...
    static void setup() throws Exception {
        ...
        abuserHistory2 = getObjectsFromDB(
                DbName.POSTGRES,
                AR_ABUSER_HISTORY_TABLE_NAME,
                String.format("ucid = '%s'", client2.getUcid()),
                AbuserHistory.class);
        holdingDeduction2 = generateAbuserDeductionByAccount(
                account2, abuserHistory2.getLast().getId());
    ...
    void holdingSchedulerSendToLarkTest() throws Exception {
        executeQueryToDb(
                DbName.CLICKHOUSE,
                String.format(
                        "UPDATE %s SET is_deleted = 1 WHERE account = %s", MT5_POSITIONS_TABLE_NAME, account.account));
        ...
        for (int i = 0; i < 10; i++) {
            deductionList = getObjectsFromDB(
                    ...
                    String.format("account = '%s'", account.account),
        ...
        assertThat(
                "Verify statusOpenPositions",
                wasHoldingDeduction.getStatusOpenPositions(),
                is(WAS_HOLDING.getDisplayName()));
        assertThat(
                "Verify statusDeduction",
                wasHoldingDeduction.getStatusDeduction(),
                anyOf(
                        is(TO_BE_DEDUCTED.getDisplayName()),
                        is(PROCESSING.getDisplayName()),
                        is(FAILED.getDisplayName())));
        ...
        assertThat(
                "Verify balanceAtResolution",
                wasHoldingDeduction.getBalanceAtResolution(),
                is(coercedObject.getProfit()));
        // Verify the deduction after deduction is picked up for sending to kafka
        for (int i = 0; i < 7; i++) {
            deductionList = getObjectsFromDB(
                    DbName.POSTGRES,
                    AR_ABUSER_DEDUCTION_TABLE_NAME,
                    String.format("account = '%s'", coercedObject.getAccount()),
                    AbuserDeduction.class);
        ...
        String kafkaRequestId =
                ArHelper.deleteDeductionsKafkaResponse(client.getUcid()).getFirst();

        executeQueryToDb(
                DbName.POSTGRES,
                String.format(
                        "UPDATE %s SET status_deduction = '%s' WHERE account = '%s'",
                        AR_ABUSER_DEDUCTION_TABLE_NAME, PROCESSING.getDisplayName(), account.account));

        // Produce AccountDeductionRequestResponse for holdingDeduction2

        AccountDeductionRequestResponse response =
                new AccountDeductionRequestResponse(Instant.now().toString(), kafkaRequestId, "Success", "");
        kafka.produceMessage(
                null, objectMapper.writeValueAsString(response), KAFKA_TOPIC_ACCOUNT_DEDUCTION_REQUEST_RESPONSE);
        AbuserDeduction finalizedDeduction = getObjectsFromDB(
                        DbName.POSTGRES,
                        AR_ABUSER_DEDUCTION_TABLE_NAME,
                        String.format("account = '%s'", account.account),
                        AbuserDeduction.class)
        ...
        ChatHistoryResponse responseLark =
        ...
        List<ChatHistoryResponse.LarkApiDataItem> itemsFiltered = items.stream()
                .filter(i ->
                        i.getBody().getContent().contains(client.getUserId().toString()))
                .toList();
        ...
        assertEquals(
                ": " + client.getUserId(),
                message.getElements().getFirst().get(5).getText());
        Allure.step("check that message contains Client id");
        assertEquals(
                ": " + client.getBrand(),
                message.getElements().getFirst().get(7).getText());
        Allure.step("check that message contains Client id");
        assertEquals(
                ": " + finalizedDeduction.getActualDeduction() + " " + account.currency,
                message.getElements().getFirst().get(9).getText());
    }
    ...
    void holdingSchedulerNoSendToLarkTest() throws Exception {
        executeQueryToDb(
                DbName.CLICKHOUSE,
                String.format(
                        "UPDATE %s SET profit=5000 WHERE account = %s",
                        MT4_TRADES_COERCED_TABLE_NAME, account2.account));
        coercedObject2.setProfit(5000.0);
        executeQueryToDb(
                DbName.CLICKHOUSE,
                String.format(
                        "UPDATE %s SET is_deleted = 1 WHERE account = %s", MT5_POSITIONS_TABLE_NAME, account2.account));
        // Verify the deduction after positions are closed
        List<AbuserDeduction> deductionList = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            deductionList = getObjectsFromDB(
                    DbName.POSTGRES,
                    AR_ABUSER_DEDUCTION_TABLE_NAME,
                    String.format("account = '%s'", account2.account),
                    AbuserDeduction.class);
            if (!Objects.equals(deductionList.getFirst().getStatusOpenPositions(), WAS_HOLDING.getDisplayName())) {
                Thread.sleep(2000);
                if (i == 9) {
