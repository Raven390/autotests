...
class RemoveRestrictionsAfterDeductionRespondedTest extends TestBaseWeb {
    ...
    @Test
    @DisplayName(
            "restrictions WR and Withdrawals automatically removed after deduction processed and got response in Kafka")
    @AllureId("1522")
    void restrictionsRemovesWhenDeductionGetResponse() throws Exception {
        ...
        DeductionKafkaRequest deductionRequestDb = (getObjectsFromDB(
                        DbName.POSTGRES,
                        AR_DEDUCTION_KAFKA_REQUEST_TABLE_NAME,
                        "payload like '%" + client.getTradingAccount() + "%'",
                        DeductionKafkaRequest.class))
                .getFirst();
        AccountDeductionRequest request =
                objectMapper.readValue(deductionRequestDb.getPayload(), AccountDeductionRequest.class);
        ...
        kafka.produceMessage(
                ...
                KAFKA_TOPIC_ACCOUNT_DEDUCTION_REQUEST_RESPONSE);
        List<EventOld> cancellationEvents = getObjectsFromDB(
                DbName.POSTGRES,
                AUDIT_EVENT_OLD,
                "ucid = '" + client.getUcid() + "' and type = 'RESTRICTION_CANCELLED'",
                EventOld.class);
        ...
        EventOld eventFirst = cancellationEvents.stream()
                .filter(e -> "Withdrawals".equals(e.getDetails()))
                .findFirst()
                .orElse(null);
        ...
        EventOld eventSecond = cancellationEvents.stream()
                .filter(e -> "Manual withdrawal review".equals(e.getDetails()))
                .findFirst()
                .orElse(null);
