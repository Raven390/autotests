...
class MirrorTradingInternalHedgeRuleTests extends TestBaseRule {
    ...
    @Test
    @AllureId("1898")
    @DisplayName("Mirror trade internal hedge. Exit without alert if (Is deal profit of the positive leg > 50$?) = false. ElementId: EVENT_END_NO_ALERT")
    void mirrorTradeInternalHedgeTest1() throws Exception {
        DataHelper data = dbDataMap.get("1");

        produceInternalHedgeMessageToKafka(data.internalHedgeEvent);

        checkElementId("EVENT_END_NO_ALERT", data.internalHedgeEvent.getId(), MIRROR_TRADE_INTERNAL_HEDGE.getProcessId());
    }

    @Test
    @AllureId("1899")
    @DisplayName("Mirror trade internal hedge. Exit with alert if (Is deal profit of the positive leg > 50$?) = true. ElementId: EVENT_END_WITH_ALERT")
    void mirrorTradeInternalHedgeTest2() throws Exception {
        DataHelper data = dbDataMap.get("2");

        produceInternalHedgeMessageToKafka(data.internalHedgeEvent);

        checkElementId("EVENT_END_WITH_ALERT", data.internalHedgeEvent.getId(), MIRROR_TRADE_INTERNAL_HEDGE.getProcessId());
        ...
        assertThat("Verify alert", alerts.getFirst().getServerId(), is(data.internalHedgeEvent.getPositiveLeg().getServerId().toString()));
