...
public class EventGeneratorLoginRequiredParametersTests {
    ...
    static Integer userId = getRandomIntPositive();
    ...
    private static Stream<Arguments> loginDbEventDataRequiredParameters() {
        return Stream.of(
                Arguments.of(null, getRandomIntPositive(), brand, ipAddress, uaString, cookie),
                Arguments.of(loginDatetime, null, brand, ipAddress, uaString, cookie),
                Arguments.of(loginDatetime, getRandomIntPositive(), null, ipAddress, uaString, cookie));
    }
    ...
    private static Stream<Arguments> loginDbEventDataNotRequiredParameters() {
        return Stream.of(
                Arguments.of(loginDatetime, getRandomIntPositive(), brand, null, uaString, cookie),
                Arguments.of(loginDatetime, getRandomIntPositive(), brand, ipAddress, null, cookie),
                Arguments.of(loginDatetime, getRandomIntPositive(), brand, ipAddress, uaString, null));
    }
    ...
    private static Stream<Arguments> loginDbEventMetadataNotRequiredParameters() {
        return Stream.of(Arguments.of(null, recordType, operation, partitionKeyType, schemaName, tableName),
    ...
    @Tag(LAYER_API)
    @AllureId("111")
    public void generateLoginEventTest1(String loginDatetime, Integer userId, String brand, String ipAddress,
            String uaString, String cookie) throws JsonProcessingException, InterruptedException {

        LoginDbEventData data = getLoginDbEventData(loginDatetime, userId, brand, ipAddress, uaString, cookie);
        LoginDbEventMetadata metadata = getLoginDbEventMetadata(timestamp, recordType, operation, partitionKeyType,
                schemaName, tableName);

        Allure.step("Write message to crm-db-events topic");
        LoginDbEvent crmDbEvent = LoginDbEvent.getLoginDbEvent(data, metadata);
        kafka.produceMessage(TAG_AUTOMATION, objectMapper.writeValueAsString(crmDbEvent), KAFKA_TOPIC_CRM_DB_EVENTS);

        Allure.step("Wait for event generator do some magic and consume message from crm-events topic");
        String consumedMessage = kafka.consumeMessage(KAFKA_TOPIC_CRM_EVENTS, String.valueOf(timestamp));
    ...
    @Tag(LAYER_API)
    @AllureId("112")
    public void generateLoginEventTest2(String loginDatetime, Integer userId, String brand, String ipAddress,
            String uaString, String cookie) throws JsonProcessingException, InterruptedException {
        LoginDbEventData data = getLoginDbEventData(loginDatetime, userId, brand, ipAddress, uaString, cookie);
        LoginDbEventMetadata metadata = getLoginDbEventMetadata(timestamp, recordType, operation, partitionKeyType,
                schemaName, tableName);
    ...
    @Tag(LAYER_API)
    @AllureId("113")
    public void generateLoginEventTest5(String timestamp, String recordType, String operation, String partitionKeyType,
            String schemaName, String tableName) throws JsonProcessingException, InterruptedException {
    ...
    @Tag(LAYER_API)
    @AllureId("114")
    public void generateLoginEventTest6(String timestamp, String recordType, String operation, String partitionKeyType,
            String schemaName, String tableName) throws JsonProcessingException, InterruptedException {
