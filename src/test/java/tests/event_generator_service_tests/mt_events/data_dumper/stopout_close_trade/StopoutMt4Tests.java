...
class StopoutMt4Tests extends TestBaseKafka {
    ...
    @Tag("CSV-1280")
    @DisplayName(
            "Generate stopout event with event generator service from MT4 data dumper source with all fields populated")
    void generateMt4StopoutEventTest() throws JsonProcessingException, InterruptedException {
        ...
        kafka.produceMessage(
                KAFKA_MESSAGE_KEY, objectMapper.writeValueAsString(stopoutTradeMt4), KAFKA_TOPIC_MT_4_TRADE_RECORD);
        ...
        MessageWithHeaders consumedMessage = kafka.consumeMessage(
                ...
                String.valueOf(stopoutTradeMt4.getPayload().getLogin()),
                true);
        ...
        assertThat(
                "Check tradeId",
                retrievedStopoutTradeMtEvent.tradeId,
                equalTo(stopoutTradeMt4.getPayload().getOrder()));
        assertThat(
                "Check symbol",
                retrievedStopoutTradeMtEvent.symbol,
                equalTo(stopoutTradeMt4.getPayload().getSymbol()));
        assertThat("Check id", retrievedStopoutTradeMtEvent.id, instanceOf(String.class));
        assertThat(
                "Check serverId",
                retrievedStopoutTradeMtEvent.serverId,
                equalTo(stopoutTradeMt4.getHeader().getServerId()));
        assertThat(
                "Check tradingAccount",
                retrievedStopoutTradeMtEvent.tradingAccount,
                equalTo(stopoutTradeMt4.getPayload().getLogin()));
        assertThat(
                "Check volume",
                retrievedStopoutTradeMtEvent.volume,
                equalTo(stopoutTradeMt4.getPayload().getVolume()));
        assertThat(
                "Check equity",
                retrievedStopoutTradeMtEvent.equity,
                equalTo(stopoutTradeMt4.getPayload().getEquity()));
        assertThat(
                "Check balance",
                retrievedStopoutTradeMtEvent.balance,
                equalTo(stopoutTradeMt4.getPayload().getBalance()));
        assertThat(
                "Check leverage",
                retrievedStopoutTradeMtEvent.leverage,
                equalTo(stopoutTradeMt4.getPayload().getLeverage()));
        assertThat(
                "Check margin",
                retrievedStopoutTradeMtEvent.margin,
                equalTo(stopoutTradeMt4.getPayload().getMargin()));
        assertThat(
                "Check freeMargin",
                retrievedStopoutTradeMtEvent.freeMargin,
                equalTo(stopoutTradeMt4.getPayload().getFreeMargin()));
