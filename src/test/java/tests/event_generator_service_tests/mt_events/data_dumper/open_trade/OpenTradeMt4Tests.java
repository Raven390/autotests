...
class OpenTradeMt4Tests extends TestBaseKafka {
    ...
    @Tag("CSV-1280")
    @DisplayName(
            "Generate open event with event generator service from MT4 data dumper source with all fields populated")
    void generateMt4OpenTradeEventTest() throws JsonProcessingException, InterruptedException {
        ...
        assertThat(
                ...
                equalTo(openTradeMt4.getPayload().getOrder()));
        assertThat(
                "Check symbol",
                retrievedOpenTradeMtEvent.symbol,
                equalTo(openTradeMt4.getPayload().getSymbol()));
        assertThat("Check id", retrievedOpenTradeMtEvent.id, instanceOf(String.class));
        assertThat(
                "Check serverId",
                retrievedOpenTradeMtEvent.serverId,
                equalTo(openTradeMt4.getHeader().getServerId()));
        assertThat(
                "Check tradingAccount",
                retrievedOpenTradeMtEvent.tradingAccount,
                equalTo(openTradeMt4.getPayload().getLogin()));
        assertThat(
                "Check volume",
                retrievedOpenTradeMtEvent.volume,
                equalTo(openTradeMt4.getPayload().getVolume()));
        ...
        assertThat(
                ...
                equalTo(openTradeMt4.getPayload().getEquity()));
        assertThat(
                "Check balance",
                retrievedOpenTradeMtEvent.balance,
                equalTo(openTradeMt4.getPayload().getBalance()));
        assertThat(
                "Check leverage",
                retrievedOpenTradeMtEvent.leverage,
                equalTo(openTradeMt4.getPayload().getLeverage()));
        assertThat(
                "Check margin",
                retrievedOpenTradeMtEvent.margin,
                equalTo(openTradeMt4.getPayload().getMargin()));
        assertThat(
                "Check freeMargin",
                retrievedOpenTradeMtEvent.freeMargin,
                equalTo(openTradeMt4.getPayload().getFreeMargin()));
